export class MediaSorter {
    ...
    bindMethods() {
        ...
        this.listboxData = this.listboxData.bind(this);
    }
    ...
    handleKeyUpButtonListbox(event) {
        ...
        const shortcutsAllowed = this.listboxData().map((item) => item.key);
    ...
    handleKeyDownListbox(event) {
        ...
        if (event.key === 'ArrowUp') {
            const nextTarget = event.target === this.listElement.firstChild ? this.listElement.lastChild : event.target.previousSibling;
            nextTarget.focus();
        } else if (event.key === 'ArrowDown') {
            const nextTarget = event.target === this.listElement.lastChild ? this.listElement.firstChild : event.target.nextElementSibling;
            nextTarget.focus();
        }
    }
     ...
     */
    buildComponent() {
        const listboxData = this.listboxData();
